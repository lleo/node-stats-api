...
NameSpace.prototype.reset = function(){
  u.each(this.ns, function(v,k){
    assert(v instanceof Stat ||
           v instanceof Histogram ||
           v instanceof NameSpace
        , "value is not a Stat, Histogram, nor NameSpace object")
    v.reset()
  })
}
...
var MovingAverage = Stats.MovingAverage = function MovingAverage(opt){
  ...
  if (!hasOwnProperty(opt, 'units')) opt.units = opt.stat.units
  Value.call(this, opt)
