...
NameSpace.prototype.register = function NameSpace__register(name, s) {
  if (this.ns[name] !== undefined)
    throw new Error(format("'%s' already registered", name))

  this.ns[name] = s
  this.emit('registered', name, s)

  return this
}

NameSpace.prototype.unregister = function NameSpace__unregister(name) {
  var s = this.ns[name]
  if (delete this.ns[name] && s !== undefined) {
    this.emit('unregistered', name, s)
  }
  return s
}

NameSpace.prototype.get = function NameSpace__get(name) {
  return this.ns[name]
}
...
function NameSpace__createStat(name, type, opt) {
  if (isString(type)) {
    if (type in NameSpace.statTypeMap)
      type = NameSpace.statTypeMap[type]
    else
      throw new Error(format("unknown stat type '%s'", type))
  }
...
function NameSpace__createHistogram(name, stat, bucketer, opt) {
  if (isString(stat)) {
    stat = this.get(stat)
  }
  if (isString(bucketer)) {
    if (bucketer in NameSpace.bucketerTypeMap)
      bucketer = NameSpace.bucketerTypeMap[bucketer]
    else
      throw new Error(format("unknown bucketer type '%s'", bucketer))

  }

  var bkter = new bucketer(opt)
 ...
 *
 */
var Value = exports.Value = function Value(units) {
  Stat.call(this)
  this.last = undefined
  this.units = units || ""
}
...
NameSpace.statTypeMap['Value'] = Value

Value.prototype.set = function Value__set(v) {
  this.last = v
  return this.publish(null, this.last)
}

Value.DEFAULT_DISPLAY_OPT = {indent:""}
Value.prototype.toString = function Value__toString(opt) {
  opt = u.defaults(opt||{}, Value.DEFAULT_DISPLAY_OPT)
  var str = opt.indent + this.last+""
...
NameSpace.statTypeMap['Timer'] = Timer
...
Timer.prototype.start = function Timer__start() {
  ...
  return function(err) {
    self.last = Date.now() - t0
    self.publish(err, self.last)
  }
}

Timer.DEFAULT_DISPLAY_OPT = {indent:""}
Timer.prototype.toString = function Timer__toString(opt) {
  opt = u.defaults(opt||{}, Timer.DEFAULT_DISPLAY_OPT)
  var str = opt.indent + this.last+""
  return str
}
...
function MovingAverage(opts) {
  Stat.call(this)

  assert(opts.hasOwnProperty('stat'), "require option 'stat' missing")
  var stat = opts.stat

  this.last = undefined
  this.nelts = opts && opts.nelts || 10
  this.values = []
  
  var self = this
  if (stat instanceof Stat) {
    this.consume_stat = stat
    this.consume_cb = function(v) { self.add(v) }
    stat.on('value', this.consume_cb)
  }
  else
    throw new Error("given stat is not of type Stat: "+inspect(stat))
}
util.inherits(MovingAverage, Stat)

NameSpace.statTypeMap['MovingAverage'] = MovingAverage
...
MovingAverage.prototype.add = function MovingAverage__add(value) {
  ...
  if (this.values.length < this.nelts) {
    ...
    this.last = sum / this.values.length
  }
  else {
    //this.last = this.last - (this.values[0]/this.nelts) + (value/this.nelts)

    var oldest = this.values[0]
      , newest = value

    //this.last -= this.values[0]/this.nelts
    //this.last += value/this.nelts
    this.last -= oldest/this.nelts
    this.last += newest/this.nelts
...
MovingAverage.prototype.destroy = function MovingAverage__destroy() {
  if (this.consume_stat)
    this.consume_stat.removeListener('value', this.consume_cb)
}

MovingAverage.DEFAULT_DISPLAY_OPT = {indent:""}
MovingAverage.prototype.toString = function MovingAverage__toString(opt) {
  opt = u.defaults(opt||{}, MovingAverage.DEFAULT_DISPLAY_OPT)
  var str = opt.indent + this.last+""
  return str
}
...
NameSpace.statTypeMap['RunningAverage'] = RunningAverage

RunningAverage.prototype.add = function RunningAverage__add(value) {
  if (this.last === undefined)
    this.last = value
  else
    this.last = (this.last*(this.nelts-1) + value)/this.nelts
...
RunningAverage.DEFAULT_DISPLAY_OPT = {indent:""}
RunningAverage.prototype.toString = function RunningAverage__toString(opt) {
  opt = u.defaults(opt||{}, RunningAverage.DEFAULT_DISPLAY_OPT)
  var str = opt.indent + this.last+""
  return str
}
...
var Linear = exports.Linear = function Linear(opts) {
  Bucketer.call(this)
  this.base  = opts && opts.base  || 1
  this.units = opts && opts.units || ""
}
util.inherits(Linear, Bucketer)

NameSpace.bucketerTypeMap['Linear'] = Linear
...
NameSpace.bucketerTypeMap['LogMS'] = LogMS
...
NameSpace.bucketerTypeMap['SemiLogMS'] = SemiLogMS
...
NameSpace.bucketerTypeMap['LinLogMS'] = LinLogMS
...
NameSpace.bucketerTypeMap['Bytes'] = Bytes
...
NameSpace.bucketerTypeMap['SemiBytes'] = SemiBytes
...
NameSpace.bucketerTypeMap['LogBytes'] = LogBytes
