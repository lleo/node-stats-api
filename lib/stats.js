...
/*
 * var max10 = Math.floor(Math.log(Number.MAX_VALUE)/Math.LN10) //=> 308
 * for (var n=0; n < max10; n++) {
 *   var r = Math.log(Math.pow(10,n))/Math.LN10
 *    if (r!=n) console.log("n=%d, 1-r%1=%d", n, 1-r%1)
 * }
 *
 * var min10 = Math.ceil(Math.log(Number.MIN_VALUE)/Math.LN10) //=> -323
 * for (var n=-1; n > min10; n--) {
 *   var r = Math.log(Math.pow(10,n))/Math.LN10
 *   if (r!=n) console.log("n=%d, 1+r%1=%d", n, 1+r%1)
 * }
 *
...
function order10(v) {
  if (v === 0) return 0 //FIXME this is stupid
  return Math.floor( log10(v) )
}

/*
 * var max2 = Math.log(Number.MAX_VALUE)/Math.LN2 //=> 1024
 * for (var n=0; n < max2; n++) {
 *   var r = Math.log(Math.pow(2,n))/Math.LN2
 *   if (r!=n) console.log("n=%d, r%1=%d", n, r%1)
 * }
 *
 * var min2 = Math.log(Number.MIN_VALUE)/Math.LN2 //=>1074
 *   , v=.5
 * for (n=-2; n > min2; n--){
 *   v*=.5
 *   r = Math.log(v)/Math.LN2
 *   if (r!=n) console.log("n=%d, r=%d, r%1=%d", n, r, r%1)
 * }
 */
function log2(v) {
...
function order2(v) {
//FIXED  if (v==0) return 0 //FIXME this is stupid
  //FIXME for v < 1 should be
 ...
 *
 */
Stats.NameSpace = Stats.NS = NameSpace
function NameSpace(){
...
NameSpace.prototype.reset = function(){
  u.each(this.stats, function(v,k){
    assert(v instanceof Stat, "value is not a Stat object")
    v.reset()
  })
  u.each(this.hogs, function(v,k){
    assert(v instanceof Histogram, "value is not a Histogram object")
    v.reset()
  })
  u.each(this.nss, function(v,k){
    assert(v instanceof NameSpace, "value is not a NameSpace object")
    v.reset()
  })
}

NameSpace.prototype.set          = registerAny
NameSpace.prototype.register     = registerAny
function registerAny(name, thing){
  assert(typeof name === 'string', "name MUST be a string")

  if (thing instanceof Stat)
    this.setStat(name, thing)
  else if (thing instanceof Histogram)
    this.setHOG(name, thing)
  else if (thing instanceof NameSpace)
    this.setNS(name, thing)
  else
    throw new TypeError("thing is not a Stat, Histogram, nor NameSpace object")

  return
}

NameSpace.prototype.unset      = unregisterAny
NameSpace.prototype.unregister = unregisterAny
function unregisterAny(name){
  var thing

  if (has(this.stats, name))
    return this.unsetStat(name)
  if (has(this.hogs, name))
    return this.unsetHOG(name)
  if (has(this.nss, name))
    return this.unsetNS(name)

  return undefined
}

NameSpace.prototype.setStat      = registerStat
NameSpace.prototype.registerStat = registerStat
function registerStat(name, stat){
  assert(typeof name === 'string', "name MUST be a string")
  assert(stat instanceof Stat, "thing is not a Stat object")

  this.stats[name] = stat

  this.emit('registered', name, stat)

  return this
}

NameSpace.prototype.setHOG            = registerHistogram
NameSpace.prototype.registerHOG       = registerHistogram
NameSpace.prototype.setHistogram      = registerHistogram
NameSpace.prototype.registerHistogram = registerHistogram
function registerHistogram(name, hog){
  assert(typeof name === 'string', "name MUST be a string")
  assert(hog instanceof Histogram, "thing is not a Stat object")

  this.hogs[name] = hog

  this.emit('registered', name, hog)

  return this
}

NameSpace.prototype.setNS             = registerNameSpace
NameSpace.prototype.registerNS        = registerNameSpace
NameSpace.prototype.setNameSpace      = registerNameSpace
NameSpace.prototype.registerNameSpace = registerNameSpace
function registerNameSpace(name, ns){
  assert(typeof name === 'string', "name MUST be a string")
  assert(ns instanceof NameSpace, "thing is not a NameSpace object")

  this.nss[name] = ns

  this.emit('registered', name, ns)
...
NameSpace.prototype.get = function get(name){
  return this.getStat(name) || this.getHOG(name) || this.getNS(name)
}


NameSpace.prototype.getStat = getStat
function getStat(name){
  return this.stats[name]
}

NameSpace.prototype.getHog = getHistogram
NameSpace.prototype.getHOG = getHistogram
NameSpace.prototype.getHistogram = getHistogram
function getHistogram(name){
  return this.hogs[name]
}

NameSpace.prototype.getNS = getNameSpace
NameSpace.prototype.getNameSpace = getNameSpace
function getNameSpace(name){
  return this.nss[name]
}

NameSpace.prototype.createStat = createStat
function createStat(name, type, opt){
  assert(!has(this.stats, name), format("name, %s, already used", name))
  assert(inheritsFrom(type, Stat), "type argument does not inherit from Stat")

  var stat = new type(opt)
  this.setStat(name, stat)
  return stat
}

NameSpace.prototype.createHog       = createHistogram
NameSpace.prototype.createHOG       = createHistogram
NameSpace.prototype.createHistogram = createHistogram
function createHistogram(name, stat, bucketer, opt){
  assert(!has(this.hogs, name), format("name, %s, already used", name))
  ...
  this.setHOG(name, hog)

  return hog
}

NameSpace.prototype.createNS        = createNameSpace
NameSpace.prototype.createNameSpace = createNameSpace
function createNameSpace(name){
  assert(!has(this.nss, name), format("name, %s, already used", name))
...
//          })
  ...
  stats = Object.keys(this.stats)
          .map(function(name){ return [ name, self.getStat(name) ] })
  hogs  = Object.keys(this.hogs)
          .map(function(name){ return [ name, self.getHOG(name)  ] })
  nss   = Object.keys(this.nss)
          .map(function(name){ return [ name, self.getNS(name)   ] })
...
var Value = Stats.Value = function Value(opt){
  ...
  assert(has(opt, 'units'), "opt.units is required")
 ...
 *
 * @param {object} [opt]
 *   opt.sigDigits   : number : default 9
 *   opt.fractDigits : number : default 2
 *   opt.commify     : boolean: default true
 */
Object.defineProperty(Value.prototype, 'DEFAULT_DISPLAY_OPT'
                     , { value: { sigDigits: 9, fractDigits: 2, commify: true }
                       , writable: true
                       , configurable: true
                       , enumerable: false })
...
Value.prototype.toString = function toString(opt){
  ...
  if (typeof this.value == 'number') {
    ...
    //else if (this.value < 1e7 && this.value > 0)
    //  str = sprintf("%.*g", this.value, opt.sigDigits)
    else
      str = sprintf("%.*f", this.value, opt.fractDigits)
...
var Rate = Stats.Rate = function Rate(opt){
  ...
  //assert(has(opt, 'units'), "'units' is a required option")
  assert(has(opt, 'stat'), "'stat' is a required option")
  assert(opt.stat instanceof Stat, "'stat' must be of type Stat")

  if (!has(opt, 'units')) opt.units = ""
  Value.call(this, opt)
...
var MovingAverage = Stats.MovingAverage = function MovingAverage(opt){
  var self = this
  assert(opt, "options object argument is required")
  assert(has(opt, 'stat'), "require option 'stat' missing")
  assert(opt.stat instanceof Stat, "given stat is not of type Stat")

  if (!has(opt, 'units')) opt.units = opt.stat.units
  Value.call(this, opt)
...
MovingAverage.prototype.reset = function(){
  var ovalue = this._value
  this._value = 0
  this.values = []
  this.emit('reset')
  return ovalue
...
var linearKB = Stats.linearKB = new LinearBucketer(1024, 'KB')
